...
c		xovmax - an upper bound on delta for which the direct ray can be the first arrival
c
c	internal arrays:
c		tr(m) - travel time for refraction in layer m
c		tinj(m) - traveltime intercept
c		tid(m) - terms in travel time intercept which are independent of tkj
c		didj(m) - critical distance
c		did(m) - terms in critical distance which are independent of tkj
c
c
